export function Matrix(canvas, adjacency, canvasProperties) {
    let matrix = {
        ...
        draw: function(options) {
            ...
            links.forEach((link) => {
                matrix[link.source * dimension + link.target].weight = link.weight;
                matrix[link.target * dimension + link.source].weight = link.weight;
            ...
            function cellColor(d) {
                if(selected == null) {
                    return (d.z) ? colorScale(d.weight) : 'none';
                }
                if(d.x == selected || d.y == selected) return (d.z) ? colorScale(d.weight) : 'darkgray';
                return (d.z) ? colorScale(d.weight) : 'none';
            }
